...
bbox_gen_question_description = '''Please pinpoint the bounding box coordinates of the element’s region that I should click in order to complete the following instruction: \"{instruction}\"'''

cogagent_question_description = '''What's the proper procedure for \"{instruction}\" (with grounding). '''
...
def generate_prompt(experiment_split, task=None, previous=None, choices=None):
    ...
    if experiment_split in ["text","text_choice","4api"]:
    ...
    elif experiment_split in ["bbox_generate"]:
        system_prompt_input = exp4_prompt_dict["system_prompt"]

        sysprompt, query_text = generate_new_query_prompt(system_prompt=system_prompt_input, task=task, previous_actions=previous,
                                      question_description="")

        query_text = query_text.replace('You are asked to complete the following task: ','')
                                    
        query_text = bbox_gen_question_description.format(instruction=query_text)

        return [sysprompt,query_text]

    elif experiment_split in ["bbox_generate_cogagent"]:
        system_prompt_input = ""
        ...
        query_text = cogagent_question_description.format(instruction=query_text)
